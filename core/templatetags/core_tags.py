...
#Public CCCT Affiliates
@register.inclusion_tag(
    'tags/our_affiliates.html',
    takes_context=True
)
def our_affiliates(context, count=5):
    affiliations = Affilliation.objects.filter(is_active=True)
    return {
        'affiliations': affiliations[:count],
        # required by the pageurl tag that we want to use within this template
        'request': context['request'],
    }
